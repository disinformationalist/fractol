...
static void	set_gauss_kernel(t_filter *f, int kern_size, double sigma)
{
	int i;
	int j;
	int center;
...
int	make_gaussian(t_filter *f, int kern_size, double sigma)
{
	if (malloc_kernel(f, kern_size) == -1)
		return (-1);
	set_gauss_kernel(f, kern_size, sigma);
...
/* int	gaussian_blur_matrix(void *mlx_ptr, double **mat, int width, int height, int kern_size, double sigma)
{
	t_filter	f;
	t_img		*img_pad;
