...
int	mean_convo_filter(void *mlx_ptr, t_img *img, int width, int height, int kern_size)
{
	...
	while (++f.y < height + f.half_k)
	{
		...
		while (++f.x < width + f.half_k)
			mean_filter_section(img_pad, img, kern_size, &f);
	}
...
static void	mean_filter_section_mat(double **mat_pad, double **mat, int kern_size, int half_k, int x, int y)
{
	double sum = 0.0;
	double avg;
	double area = SQ(kern_size);
	int i, j;
	...
	avg = sum / area;
	//if (avg > mat[y - half_k][x - half_k])
		mat[y - half_k][x - half_k] = avg;
}
...
int	mean_convo_matrix(double **mat, int width, int height, int kern_size)
{
	...
	half_k = kern_size / 2;
	mat_pad = pad_mirror_matrix(mat, width, height, half_k);
	if (mat_pad == NULL)
	{
		write(2,"pad_mat failure\n", 17);
		return (-1);
	}
	y = half_k - 1;
	while (++y < height + half_k)
	{
		...
		while (++x < width + half_k)
			mean_filter_section_mat(mat_pad, mat, kern_size, half_k, x, y);
	}
	free_matrix_i(mat_pad, height + half_k * 2);
	return (0);
}